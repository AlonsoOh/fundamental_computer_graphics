...
void display(void) {
	...
	for (int i = -360; i <= 360; i += 90) {
		float angle = (float)i;
		ModelViewProjectionMatrix = glm::translate(ViewProjectionMatrix, glm::vec3(sin(-angle * TO_RADIAN), 0.0f, angle * TO_RADIAN));
	...
	/*for (int i = 0; i < 360; i += 60) {
		...
		ModelViewProjectionMatrix = glm::translate(ViewProjectionMatrix, 
			glm::vec3(3 * cos(angle * TO_RADIAN), 3 * sin(angle * TO_RADIAN), 0.0f));
		ModelViewProjectionMatrix = glm::rotate(ModelViewProjectionMatrix, angle * TO_RADIAN, glm::vec3(0.0f, 1.0f, 0.0f));
