...
void display(void) {
	...
	for (int i = 0; i <= 360; i += 60) {
		float angle = (float)i;
		ModelViewProjectionMatrix = glm::translate(ViewProjectionMatrix, glm::vec3(-3.0f + (angle / 60.0f), 0.0f, 2.0f));
		ModelViewProjectionMatrix = glm::rotate(ModelViewProjectionMatrix, angle * TO_RADIAN,
			glm::vec3(0.0f, 1.0f, 0.0f));
		//다음 주석처리한 부분은 (c)번 파트를 확인하려면 활성화시킬것! 
		//ModelViewProjectionMatrix = glm::rotate(glm::mat4(1.0f), angle * TO_RADIAN, glm::vec3(0.0f, 1.0f, 0.0f));
